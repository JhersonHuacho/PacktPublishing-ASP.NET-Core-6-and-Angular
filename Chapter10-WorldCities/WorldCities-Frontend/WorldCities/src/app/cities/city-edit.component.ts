...
export class CityEditComponent extends BaseFormComponent implements OnInit, OnDestroy {
  ...
  // unsubscribe usando un enfoque de "suscripción única" tambien conocido como forma imperativa
  private suscriptions: Subscription = new Subscription();
  // unsubscribe usando enfoque declarativo
  ...
  ngOnInit(): void {
    ...
    this.suscriptions.add(this.form.valueChanges.subscribe(() => {
      if (!this.form.dirty) {
    ...
    this.suscriptions.add(this.form.get("name")!.valueChanges.subscribe(() => {
      if (!this.form.dirty) {
    ...
    // react to changes in the form.name lat
    this.form.get("lat")!.valueChanges
      .pipe(takeUntil(this.destroySubject))
      .subscribe(() => {
        if (!this.form.dirty) {
          this.log("Lat has been loaded with initial values.");
        }
        else {
          this.log("Lat was updated by the user.");
        }
      });
  ...
  ngOnDestroy(): void {
    // primera forma de desuscribirse
    this.suscriptions.unsubscribe();

    // segunda forma de desuscribirse
    // emit a value with the takeuntil notifier
    this.destroySubject.next(true);
    // complete the subject to stop listening
